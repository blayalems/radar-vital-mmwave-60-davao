...
| Component | File | Role |
|---|---|---|
| **Firmware** | [`radar_vital_v16_6_4.ino`](./radar_vital_v16_6_4.ino) | XIAO ESP32-C6 + MR60BHA2 driver. Emits the v15.2 222-column CSV at 115 200 baud over USB; the first 207 columns remain the frozen v15 contract, 208-219 preserve v15.1 diagnostics, and 220-222 retain the audit fields introduced in v16.4. `ENABLE_BLE` stays off by default; passive module-firmware readback after radar boot/recovery lets session truthfulness record the MR60BHA2 version. |
...
- **Firmware readback**: `radar_vital_v16_6_4.ino` passively polls the MR60BHA2 module firmware version immediately after `mmWave.begin()` and after radar recovery, so captures can populate `module_fw_*` / `module_fw_valid`.
...
last-known-good backup. If that store is unavailable, new profile creation and
PIN or recovery-code changes fail with `503 OPERATOR_STORE_UNAVAILABLE` and do
not report success. New sign-in also fails closed when credentials cannot be
read. Already-issued, unexpired operator sessions remain usable so an active
capture can be stopped safely; restore writable profile storage before making
credential changes or starting a new sign-in.
...
For held-out model evidence, enable `--loo-eval` so each run writes
`outer_oof_predictions.csv` with raw and causal post-processed predictions.
Generate the proposal-aligned participant-balanced report with the versioned
plan and manuscript-ready exports:

```bash
rvt-statistics --input model-gbr/outer_oof_predictions.csv \
  --reference-column ref_rr --estimate-column pred_rr \
  --participant-column participant_id --tost-margin 2 \
  --analysis-plan quality/statistical-analysis-plan.json --confirmatory \
  --out results/statistical_report.json \
  --csv-out results/statistical_report.csv \
  --latex-out results/statistical_report.tex
```

The analysis plan is draft-controlled pending research/quality approval. It
freezes the 0.6/0.8/1.0 m × none/cardboard × three 150-second protocol, the
1.0 m/no-cardboard primary RR TOST, five Holm-adjusted secondary tests, and
the minimum-window/trial aggregation rules.

Confirmatory artifacts keep five identities separate:
`run_product_version`, `analysis_plan_id`, `analysis_plan_sha256`,
`study_protocol_id`, and `study_session_schema_version`. A new software patch
therefore does not rewrite the historical plan identity, while the canonical
plan digest prevents edited plan content from passing under an unchanged ID.
The plan remains `draft`; these software gates do not authorize recruitment,
confirmatory collection, exclusions, or thesis claims.
...
```
.
├── radar_vital_v16_6_4.ino                         # firmware (v16.6.3; v15.2 222-column USB contract, BLE gated off)
