"""Preflight and audit runner helpers for the trainer.

This module owns the real implementation of the operator preflight
wizard backing ``/api/preflight`` and ``/api/preflight/<check>``. Each
check is a small, self-contained probe (Python env, firmware contract,
serial/BLE adapters, disk, schema hash, clock) that returns a uniform
result dict via :func:`_preflight_result`; :func:`run_preflight_all`
fans the checks out and aggregates a summary.

All of the trainer-internal helpers these checks lean on
(``_preflight_result``, ``_candidate_ino_paths``, ``RADAR_LOG_COLUMNS``,
``_read_session_lock``, ``_scoring_weights_hash``, ``_iso_now``,
``_extract_firmware_data_header_tokens``, and the ``DEFAULT_*``
constants) are imported lazily inside the functions so this package
stays import-cheap and free of circular-import hazards with the
monolith, which imports these back under their legacy underscored names.
"""
...
# Canonical ordering of preflight checks. ``run_preflight_all`` runs them
# in this order; structural (no-hardware) checks come first so the report
# is useful even when the radar/oximeter aren't plugged in yet.
_PREFLIGHT_CHECK_ORDER: List[str] = [
    "python_env",
...
def run_preflight_check(check_id: str, **params) -> Dict[str, object]:
    """Run a single preflight probe and return its uniform result dict.

    Never raises: any unexpected error is captured and returned as a
    ``fail`` result so a single bad probe can't abort the wizard.
    """
...
def run_preflight_all(**params) -> Dict[str, object]:
    """Run the full preflight suite (optionally filtered) and aggregate.

    ``include`` (an iterable of check ids) restricts the run to a subset;
    omitting it runs every check in :data:`_PREFLIGHT_CHECK_ORDER`. The
    return value carries a per-status ``summary`` (with a derived
    ``overall``), the individual ``checks`` results, an ISO timestamp,
    and the resolved port/address ``refs``.
    """
    from ..monolith import DEFAULT_BLE_ADDRESS, DEFAULT_RADAR_PORT, _iso_now

    checks = list(_PREFLIGHT_CHECK_ORDER)
