...
FW = ROOT / "radar_vital_v16_6_1.ino"
...
def test_v16_6_1_product_identity_bump_preserves_v12_lineage():
    ...
    assert json.loads(text(ROOT_PACKAGE))["version"] == "16.6.1"
    assert json.loads(text(ROOT_PACKAGE_LOCK))["version"] == "16.6.1"
    assert json.loads(text(TAURI_CONF))["version"] == "16.6.1"
    assert json.loads(text(PACKAGING_TAURI_CONF))["version"] == "16.6.1"
    assert json.loads(text(PACKAGING_CAP_PACKAGE))["version"] == "16.6.1"
    assert json.loads(text(PACKAGING_CAP_PACKAGE_LOCK))["version"] == "16.6.1"
    assert re.search(r'^version = "16\.6\.1"$', text(TAURI_CARGO), re.MULTILINE)

    assert 'VERSION = "16.6.1"' in trainer
    assert 'DASHBOARD_VERSION = "16.6.1"' in trainer
    assert 'FIRMWARE_VERSION_EXPECTED = "v16.6.1"' in trainer
    assert "PRODUCT_VERSION = '16.6.1';" in app_meta
    ...
    assert '#define FW_VERSION "v16.6.1"' in firmware
...
def test_firmware_ble_contract():
    ino = text(FW)
    assert '#define FW_VERSION "v16.6.1"' in ino
