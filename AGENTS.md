...
| Path | Edit policy |
|---|---|
| `web/` | **Source of truth** for the dashboard. `web/index.html` is the shell with `<!-- BUILD:INCLUDE -->` markers. `web/styles/<id>.css` and `web/modules/<id>.js` each hold one previously-inline `<style id="...">` / `<script id="...">` block (123 blocks total). Edit these files. Then `npm run build:web` regenerates the root monolith and `www/`. |
...
3. **Trainer boots clean.** `python3 radar_vital_trainer_v12_for_v16_0.py serve --mock` exits 0 on Ctrl-C and prints the dashboard URL.
4. **Playwright smoke passes.** `npm test` runs in under 90 s and reports zero console errors.
...
6. **Lint.** Python: `python -m py_compile radar_vital_trainer_v12_for_v16_0.py`. JS: ESLint not configured (vanilla, inline) — review by hand.
