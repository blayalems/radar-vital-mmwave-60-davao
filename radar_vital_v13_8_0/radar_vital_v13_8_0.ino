...
#define FW_VERSION "v13.8.0"
#define SKETCH_VERSION_MAJOR 13
#define SKETCH_VERSION_SUB 8
...
void loop() {
  ...
  if (calibrationDone) {
    ...
    if (now-lastLogMs>=LOG_INTERVAL_MS) {
      ...
      // CSV column count widened for v13.8.0 truthfulness / schema telemetry.
      // Expected DATA payload column count after the DATA, prefix: 141. Update this comment whenever columns are added or removed.
      #define CSV_COLUMN_COUNT 141
...
#define CSVFN(v,p) do { float __csv_v = (float)(v); Serial.print(isfinite(__csv_v) ? __csv_v : -1.0f, (p)); Serial.print(','); } while (0)
      ...
      CSVI((unsigned)hrConfidenceSourceThisCycle);
      CSVI((unsigned)hrUpdateSourceThisCycle); // hr_path_source still mirrors current update source for dashboard compatibility
      ...
      Serial.println((int)(rrSeedConsistentCount >= 3 ? 1 : 0));
