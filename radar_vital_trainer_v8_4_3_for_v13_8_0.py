...
EXPECTED_RADAR_LOG_COLUMN_COUNT = 141
...
for _dashboard_candidate in _dashboard_template_candidates():
    if _dashboard_candidate.exists():
        try:
            ...
            pass
_DASHBOARD_TEMPLATE_EMBEDDED = _DASHBOARD_TEMPLATE_EMBEDDED.replace("v13.8.0 / v8.4.0", "v13.8.0 / v8.4.3")
...
def load_radar(path: str) -> pd.DataFrame:
    ...
    for col, default in (("dsp_ran_this_frame", 0), ("hr_confidence_source", 0), ("sketch_major", np.nan), ("sketch_sub", np.nan), ("sketch_mod", np.nan), ("rr_seed_from_raw_used", 0)):
        if col not in df.columns:
            df[col] = default
...
def cmd_analyse(args):
    ...
    summary = {
        ...
        "fw_truthfulness": {
            "version": "v13.7.1",
            "notes": "Expects v13.7.1 truthful phase-valid/warmup telemetry and aligned CSV schema"
        }
    }
