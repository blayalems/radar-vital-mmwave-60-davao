...
async function applyCommonRoutes(page: import('@playwright/test').Page) {
  ...
  await page.route('**/api/status', route =>
    route.fulfill({
      ...
      body: JSON.stringify({
        ok: true,
        mode: 'live',
        trainer_version: '16.5.3-test',
        dashboard_version: '16.5.3-test',
        firmware_expected: 'v16.5.3-test',
        active_session: null,
        feature_flags: {}
      })
    })
  ...
  await page.route('**/api/health', route =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({ ok: true, version: '16.5.3-test' })
    })
  );
  await page.route('**/api/version', route =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({ product_version: '16.5.3-test' })
    })
  );
