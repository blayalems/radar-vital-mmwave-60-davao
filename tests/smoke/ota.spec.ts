...
test.describe('OTA Update and Version Info Smoke Tests', () => {
  ...
  test('displays product version and metadata in settings', async ({ page }) => {
    ...
    // Verify product version is rendered dynamically (should be 16.5.4 in mock/production)
    const productVersion = card.locator('.settings-copy', { hasText: 'Product Version' }).locator('.row-description');
    await expect(productVersion).toContainText('16.5.4');
  ...
  test('checks for updates successfully when a newer build of the same version is available', async ({ page }) => {
    ...
    await page.route('**/api/update/manifest', async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          artifacts: {
            apk: {
              url: 'https://github.com/blayalems/radar-vital-mmwave-60-davao/releases/download/v16.5.4-build2/radar-vital-release.apk',
              size_bytes: 15728640,
              sha256: 'a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2',
              compatibility: 'Android 8.0+'
            }
    ...
    const titleText = resultRegion.locator('.update-title-text');
    await expect(titleText).toContainText('New Build Available: v16.5.4-build2');

    // Verify snackbar alert
    await expect(page.locator('simple-snack-bar').last()).toContainText('New build of 16.5.4 is available.');
  });
