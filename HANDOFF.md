...
### 2026-06-04 — PR #44: v11→v12 parity feature closures + Phase 5 trainer extraction (genspark/frontend)

- **Universal Undo (Ctrl+Z parity)**: Added `web/src/app/services/undo.service.ts` — a bounded (depth 25), failure-safe LIFO undo stack exposing `canUndo`/`nextLabel`/`depth` signals. Wired Ctrl+Z across `live.component.ts` (snapshot delete / clear-all), `alerts-dialog.component.ts` (dismiss / acknowledge), `layout.component.ts`, and `command-palette.component.ts`, each surfacing an "Undo" snackbar. 6 unit tests in `undo.service.spec.ts`.
- **Operator Handoff brief (Ctrl+H)**: New `operator-handoff-dialog` standalone component — a structured shift-change brief (operator/station/subject/source/session/HR/RR/distance/alerts/snapshots) with clipboard copy (+textarea fallback), printable view, and a 600-char handoff note. M3 `m3-dialog-panel` styling with HC overrides. Wired into layout + command palette.
- **Idle / manual station lock (Ctrl+L)**: New `web/src/app/services/idle-lock.service.ts` — opt-in idle auto-lock (default off, 10-min timeout, clamp 1–120 min, persisted to localStorage) plus manual lock. Timers/listeners run via `NgZone.runOutsideAngular`. Full-screen scrim overlay in `layout.component` with focus-trapped Unlock button; "Privacy & Idle Lock" settings card. 7 unit tests in `idle-lock.service.spec.ts`.
- **Snapshot compare & reorder**: `live.component` snaps tab now supports reordering (up/down) and a 2-snapshot delta-compare panel (HR/RR/distance deltas, error/tertiary semantic coloring).
- **Accessible canvas data-tables**: HR / RR trend tabs gain a `table_rows` toggle that renders a sticky-header scrollable data table alternative to the canvas chart (a11y + non-visual export parity).
- **Keyboard shortcuts dialog**: Documented the now-live Ctrl+Z / Ctrl+H / Ctrl+L shortcuts (previously "not yet available").
- **Phase 5 trainer extraction**: Moved the real implementations of BLE scan, radar serial-port auto-detect, and the preflight wizard out of the 13.9k-line `monolith.py` into focused modules — `rvt_trainer/transport/ble.py` (`scan_ble_devices_payload`), `rvt_trainer/transport/serial.py` (`auto_detect_radar_port`), and `rvt_trainer/audit/runner.py` (`run_preflight_check` / `run_preflight_all`). These were previously 5-line re-export stubs; they now own the logic and use deferred monolith imports to stay circular-import-free. `monolith.py` re-exports them under the legacy underscored names so every call site is unchanged (same inversion pattern already used by `assets/static.py` and `api/sse.py`). Added 17 unit tests across `tests/test_trainer_transport.py` and `tests/test_trainer_audit.py`.
- **Verification**: Vitest **31/31** (compiles the whole Angular app — confirms all edited components type-check clean). Pytest **63/63** (46 prior + 17 new). Trainer `compileall` clean and `python -m rvt_trainer --help` exit 0; re-export wiring asserted (`__module__` checks).
- **⚠️ Known follow-up — monolith rebuild required**: The `npm run build:web` production Angular build could not complete in the 987 Mi / fully-exhausted-swap sandbox (severe OOM thrashing). Consequently the committed `radar_vital_live_dashboard_v12_for_v16_0.html` was **not** regenerated from these Angular source changes — `npm run build:check` will report monolith drift until the build is run on a machine/CI with adequate memory and the result committed. No partial build artifacts were committed (`www/` is gitignored). The source-of-truth (`web/src/`) is correct and verified by vitest.
