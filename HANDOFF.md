...
- **BLE compile diagnosis**: Hosted evidence showed both `build.extra_flags` and `compiler.cpp.extra_flags` replace required ESP32 recipe content: the first removed board identity and the second reached link with missing NimBLE OS symbols.
- **BLE link contract**: Enable BLE only in the temporary CI sketch copy, then compile with the unmodified pinned board recipe; this retains the ESP32 core's NimBLE link configuration as well as its board defines while leaving the checked-in default disabled.
...
- **Verification**: `npm run test:source-integrity`, `npm run test:qms-contract`, `npm run test:unit:sw`, `python -m pytest`, and `node scripts/build-angular.mjs` passed cleanly.
...
- **Module identity readback (server)**: truthfulness read `module_fw_major/sub/mod` after `canonicalize_with_synonyms` had renamed them to `fw_major/fw_sub/fw_mod`, so a valid module version could never be detected; `_first_populated_series` now accepts both spellings.
- **Adaptive-correction shadow (server)**: ran on the 1 Hz feature frame where radar columns are `_mean`-suffixed, so it skipped every session with "missing one of (raw_hr_uncorrected, raw_hr_corrected, ref_hr)"; now resolves suffixed names.
- **v15 PQI shadow (server)**: v15 columns are not aggregated into the 1 Hz frame, so the shadow claimed "columns not present (firmware <v15.0)" for v16.4 logs carrying sentinel -1.0; now runs on the raw radar frame and reports "sentinel — Stage C never ran" honestly.
- **BLE reference metrics (server)**: `parsed_rows/raw_packets` (~33% by oximeter protocol design) was used as coverage and its complement mislabelled `decode_error_pct` (~66% on every session, `WARN`); `_compute_ble_ref_quality` now emits time-based `coverage_pct` (seconds with >=1 parsed row / capture span) and `pi_median` (which the readiness verdict always read but never received — the PI gate was dead code printing `pi=nan`), and computes `decode_error_pct` only from real logger summary stats (NaN otherwise). Readiness prefers `coverage_pct`.
- **BLE summary survival (server)**: s08-s11 had no `ref_ble_summary.json` because the session orchestrator hard-terminates the reflog child on Windows and the summary was written only in `finally`; the summary is now also snapshotted every 5 s during logging.
- **Module firmware version (firmware `radar_vital_v16_4_0.ino`)**: `module_fw_*` was `-1`/`module_fw_valid=0` in every recorded session — the MR60BHA2 emits its version TLV around (re)boot but the first read happened after calibration + first radar frame. Added `tryReadModuleFirmwareVersion()`/`pollModuleFirmwareVersionWindow()`: setup() pumps `mmWave.update()` for <=1.5 s right after `mmWave.begin()`, and the radar-recovery path re-arms a <=400 ms poll after re-`begin()`. No new library API; passive-read semantics unchanged.
- **Data-driven follow-ups (not code-fixed here)**: HR coverage 0.8-4.4% is dominated by `NO_PHASE`/`PHASE_HOLDOFF` (salvageable <=8% per coverage ledger — DSP/placement, not gating); sessions of 36-252 s can never pass the 600 s secondary gate (collect >=10 min); `pi_below_threshold_pct` 49-100% points at oximeter finger contact during collection.
- **Tests**: new `tests/test_session_data_regressions.py` pins all five server defects.
- **Verification**: Pending in this commit — run `python -m pytest -q tests/test_session_data_regressions.py tests/test_trainer_audit.py tests/test_trainer_verdict.py`, `python -m compileall -q radar_vital_trainer_v12_for_v16_0.py rvt_trainer`, and an Arduino compile of `radar_vital_v16_4_0.ino` (authored in a no-runtime environment; session data audited from the release-artifacts `sessions/` folder).
...
- **Session logger**: Updated firmware contract lookup for `radar_vital_v16_4_0.ino`, aligned the trainer's v15.2 tail to the firmware order (`uart_rx_high_water`, publish tiers, then `fw_uptime_s`), preserved legacy v15.1 uptime padding, and let live serial logging fall back to the built-in 222-column trainer schema when packaged runtimes do not include the firmware source file.
...
- **Azure default**: `PaletteId` drops `classic`; UI store defaults to `azure`; stored `classic` migrates to `azure`; reset-defaults/prefs-import land on Azure; `data-palette` is now always set. Page canvas uses `--rv-bg` (azure light #EAF1F9) — body/app-root/sidenav/main-wrapper repaints removed.
- **v17 parity layer** (end of `styles.scss`): flat white cards (outline-variant border, `--rv-r1`, no shadow), 38px card-header icon chips + 14.5px titles, 42px/12px-radius filled inputs, muted topbar eyebrow, neutral status pill (renamed `#modeBadge`→`#statusPill` to detach ~40 legacy id skin rules), standalone 40px topbar action circles, slim in-column demo banner with outlined exit pill, v17 Simple/Advanced segment, compact Live tab chips, countered the legacy global `canvas{height:230px!important}` that truncated every graph.
- **Home**: v17 stat cards (sentence case, tinted 50px icon chips, quiet subs), label-above fields, dark #071410 radar scope, plain mono scope metrics, warn hint pill, v17 copy ("Hardware preflight", "Re-run all", "Trend — last 60 s").
- **Live**: hero/KPI cards restyled to v17 (34px advanced / 88px simple numerals, no accent stripes, conf pill beside title incl. new RR pill, mono footers); sparklines now use the v17 drawSpark spec (last-60, 2px stroke + 12% fill) in token-resolved vital colors; `HR`/`RR` tabs → `HR funnel`/`RR funnel`; primary pane fills the row when Pane B closed; primary snapshot FAB.
- **Report/Settings**: passing gates are success-green pills; Settings leads with Appearance, in-page header removed, pill search, sentence-case card titles across both screens.
- **Tests**: `test:unit:web` 157/157; `build:web` round-trip clean (initial-bundle maximumError 2.5→2.6MB after +263 B of parity CSS); desktop dashboard smoke 32/35 — the 3 remaining failures (placement-zone wording, quality-scorecard fixture, comparison `mat-select`) are stale pre-PR69 spec contracts, evidenced pre-existing via `git log -S`.
...
- **Auth bootstrap recovery**: Operator auth/profile loading no longer waits indefinitely on the initial `/api/status` probe, so the lock overlay cannot block recovery controls while status detection is pending.
- **Auth re-login stability**: Successful PIN login now clears a stale `unauthenticated` control-status marker before unlocking, so a fresh operator session is not immediately re-locked after switch/add flows.
...
- **Preflight status hardening**: Home and command-palette preflight actions now tolerate partial check payloads without throwing when a `status` field is missing.
- **Smoke auth fixture**: Dashboard smoke tests now unlock through the real onboarding/PIN UI flow when the overlay appears, avoiding stale storage-only bypasses that leave Angular's idle-lock state active.
- **CI smoke budget**: The Playwright smoke job timeout now matches the full desktop, Pixel, iPhone, and iPad matrix so a long but progressing smoke run is not cancelled at 20 minutes.
- **CI EXE toolchain**: The Tauri Windows and release-artifact workflows stay on the locally verified Rust 1.94.1 packaging toolchain while the app lockfile pins the Tauri transitive dependency set.
- **Native dependency lock**: `src-tauri/Cargo.lock` is now tracked so Windows CI does not float to the newly broken `time` 0.3.48 / `tauri-utils` 2.9.2 resolution.
- **Visual fixture stability**: Mobile/tablet Home visual baselines now pin `/api/status` to an active mock session and open the mocked SSE stream immediately, preserving baseline spacing while preventing a startup polling race from filling the progress bar in theme/layout screenshots; desktop and Pixel keep the existing live mock baseline.
- **Verification**: `npm --prefix web run test:ci` 54/54; `python -m compileall -q radar_vital_trainer_v12_for_v16_0.py rvt_trainer`; `python -m rvt_trainer --help`; `python -m pytest -q tests` 174 passed, 1 skipped (known Windows pytest temp symlink cleanup warning); `cargo test --manifest-path src-tauri/Cargo.toml --quiet` 11/11; `cargo +1.94.1 test --manifest-path src-tauri/Cargo.toml --verbose` 11/11; `cargo +1.94.1 test --manifest-path src-tauri/Cargo.toml --locked --quiet` 11/11; `npm run build:web` clean with known bundle-budget warning; `npm run build:check` clean with known bundle-budget warning; static PWA auth smoke 4/4; desktop API smoke 12/12; desktop operator smoke 1/1; iPad operator switch smoke 1/1; recovery navigation smoke 4/4; iPhone command-palette smoke 1/1; desktop dashboard console guard 1/1; desktop active-session/Home history smoke 1/1; visual Home subsets pass on desktop, Pixel 7, iPhone 14, and iPad. A full combined smoke invocation still cascaded locally into blank route pages after a long sequence, while the isolated failed route smoke re-ran green; CI remains the full matrix authority.
