...
def test_api_version_additive_product_and_schema_fields(tmp_path: Path):
    ...
    try:
        ...
        assert status == 200
        assert payload["trainer"] == "16.4.0"
        assert payload["dashboard"] == "16.4.0"
        assert payload["product_version"] == "16.4.0"
        assert payload["firmware_expected"] == "v16.4.0"
...
def test_update_manifest_proxy_success(tmp_path: Path):
    ...
    try:
        with patch("urllib.request.urlopen", side_effect=side_effect):
            ...
            assert status == 200
            assert data["product_version"] == "16.4.0"
    finally:
        server.stop()
...
def test_update_manifest_proxy_caching_and_expiration(tmp_path: Path):
    ...
    def side_effect(req, *args, **kwargs):
        ...
        if "rvt-latest.json" in url:
            ...
            mock_response = MagicMock()
            mock_data = {
                "product_version": f"16.4.0-{call_count}",
                "minimum_supported": "16.0.0",
                "released_at": "2026-06-06T00:00:00Z",
                "artifacts": {}
            }
    ...
    try:
        with patch("urllib.request.urlopen", side_effect=side_effect):
            # First request: should trigger fetch and get 16.4.0-1
            status1, data1 = _request(base, "/api/update/manifest")
            assert status1 == 200
            assert data1["product_version"] == "16.4.0-1"
            assert call_count == 1
