...
*
* =========================================================================
* CHANGELOG v13.9.6 (truthfulness + RR source semantics + trust telemetry)
* =========================================================================
* + Keep the safe fs-correctness line from v13.9.5 and the guarded branch-safe
*   behavior from v13.9d as the baseline.
* + Add explicit trust-age, dead-zone run-length, buffer-zero-injection, raw-vs-anchor,
*   raw-vs-phase, and publish-block-stage telemetry so future sessions can separate
*   physiology failure from policy failure without inference.
...
* + Refine RR source semantics: publishing can now distinguish current-loop RR source
*   success from fresh latched accepted-source continuity, and logs an explicit
*   rr_source_reject_reason instead of collapsing everything into SOURCE.
...
static const bool EXP_ENABLE_PHASE_GAP_FILL = FULL_EXPERIMENTAL_PROFILE || false;
static const bool EXP_ENABLE_CLUTTER_REWARM = FULL_EXPERIMENTAL_PROFILE || false;
...
void resetVitals() {
  ...
  rrSourceLatchedOkLogged = false; hrPublishBlockStageLogged = PUB_STAGE_OK; rrPublishBlockStageLogged = PUB_STAGE_OK;
...
void loop() {
...

      {
        ...
        if (rawEnergy < energyFloor && calibrationDone) {
            ...
            if (skipDSP_consecMisses >= SKIP_DSP_MISS_THRESH) {
              ...
              if (EXP_ENABLE_PHASE_GAP_FILL && bufCount > 0 && skipDSP_consecMisses <= 2) {
  ...
  if (EXP_ENABLE_CLUTTER_REWARM && expPostMotionHoldPrev && !rrPostMotionHold) {
  ...
  else if (!phaseBackedPublishReady) { hrPublishReason = HR_PUB_PHASE_HOLDOFF; hrPublishBlockStageLogged = PUB_STAGE_PHASE; }
  else if (!hrFreshForLog) { hrPublishReason = HR_PUB_AGE; hrPublishBlockStageLogged = PUB_STAGE_FRESHNESS; }
  ...
  else if (hrRawAvailableNow && !hrRawAgreementGood) { hrPublishReason = HR_PUB_RAW_DISAGREE; hrPublishBlockStageLogged = PUB_STAGE_RAW_OR_SOURCE; }
  ...
  else if (!phaseBackedPublishReady) { rrPublishReason = RR_PUB_HOLDOFF; rrPublishBlockStageLogged = PUB_STAGE_PHASE; }
  ...
  if (calibrationDone) {
    if (!logHeaderPrinted) {
      Serial.println("DATA,timestamp_ms,heart_phase_stabilized,breath_phase_stabilized,raw_hr,raw_rr,raw_rr_effective,raw_rr_likely_harmonic,"
                      ...
                      "near_field_reflector_suspect,agc_floor_suspect,phase_backed_publish_ready,hr_anchor_drift_suspect,phase_gap_fill_count,clutter_rewarm_count,experimental_profile_enabled,fs_effective,fs_snap_used,hr_fs_guard_min,hr_autocorr_best_conf,phase_zero_fill_pct,fs_fallback_used,module_fw_valid,hr_trust_age_ms,rr_trust_age_ms,skipdsp_run_len,agc_floor_run_len,buffer_zero_injected,hr_raw_minus_anchor_bpm,hr_phase_minus_anchor_bpm,hr_raw_minus_phase_bpm,rr_candidate_present,rr_candidate_source,rr_source_reject_reason,rr_source_latched_ok,hr_publish_block_stage,rr_publish_block_stage");
    ...
    if (now-lastLogMs>=LOG_INTERVAL_MS) {
      ...
      // CSV column count widened for v13.9.6 truthfulness / RR-source / dead-zone telemetry.
      // Expected DATA payload column count after the DATA, prefix: 177. Update this comment whenever columns are added or removed.
      #define CSV_COLUMN_COUNT 177
...
Serial.println((unsigned)rrPublishBlockStageLogged);
