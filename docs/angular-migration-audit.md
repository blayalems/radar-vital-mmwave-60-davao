...
| Native paired transport and BLE gate | [`src-tauri/src/main.rs`](../src-tauri/src/main.rs), [`web/src/app/services/bluetooth.service.ts`](../web/src/app/services/bluetooth.service.ts), [`web/src/app/components/home/home.component.ts`](../web/src/app/components/home/home.component.ts) | Rust GATT-allowlist unit test and browser-driven mocked Tauri notification probe coverage; local NSIS/APK packaging; physical GATT acceptance remains open |
| Frozen firmware protocol | [`radar_vital_v16_2_0.ino`](../radar_vital_v16_2_0.ino), [`rvt_trainer/monolith.py`](../rvt_trainer/monolith.py) | [`tests/test_v12_static_contract.py`](../tests/test_v12_static_contract.py), Arduino CLI compile |
...
- Reduce the advisory Angular initial bundle warning (currently 14.73 kB over the 2 MB warning budget) without removing migrated visual contracts; a trial removal of the legacy patch import failed desktop Home visual comparison by 52,371 pixels and was discarded.
...
- `npx playwright test tests/smoke/dashboard.spec.ts tests/smoke/api.spec.ts --reporter=line` passed 120 current checks across desktop, Pixel 7, iPhone 14 and iPad, including the dashboard-triggered mocked Tauri notification probe; Chromium verifies offline launch, while WebKit verifies precaching because Playwright offline navigation emulation reports an internal engine error.
