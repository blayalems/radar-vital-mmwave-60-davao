...
- **Verification**: `npm --prefix web run test:ci` 51/51; `python -m compileall -q radar_vital_trainer_v12_for_v16_0.py rvt_trainer`; `python -m rvt_trainer --help`; `python -m pytest -q tests` 174 passed, 1 skipped (known Windows pytest temp symlink cleanup warning); `cargo test --manifest-path src-tauri/Cargo.toml --quiet` 11/11; `cargo +1.94.1 test --manifest-path src-tauri/Cargo.toml --verbose` 11/11; `npm run build:check` clean with known bundle-budget warning; static PWA auth smoke 4/4; desktop API smoke 12/12; desktop operator smoke 1/1; desktop dashboard console guard 1/1; visual Home subsets pass on desktop, Pixel 7, iPhone 14, and iPad. A full combined smoke invocation hit the local timeout before completion, so CI remains the full matrix authority.
