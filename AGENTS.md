...
| `radar_vital_v16_6_3.ino` | Firmware. `ENABLE_BLE` is `false` by default; the active USB contract is v15.2 / 222 columns. Keep radar BLE/GATT work gated until accepted. Watchdog must never `delay()` in the DSP loop; use `bleSuppressUntilMs = millis() + N` instead. |
