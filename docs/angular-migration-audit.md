...
- Confirm current-head APK/EXE workflow runs in CI and reduce the advisory Angular initial bundle warning (currently 14.05 kB over the 2 MB warning budget).

## Verification Evidence

- `npm run build:check` passed; `web/` and the committed monolith round-trip cleanly.
- `python -m pytest -q tests` passed with 37 tests, including LAN authorization/static denial, Pages Angular-shell publication, CI snapshot-platform alignment, dark inverse HC Home ownership, and frozen 207-column/115200 protocol assertions.
