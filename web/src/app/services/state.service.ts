...
export class StateService {
  ...
  private loadFromStorage() {
    try {
      const themeVal = localStorage.getItem('rvt-theme') as any;
      if (themeVal) this.theme.set(themeVal);

      const densityVal = localStorage.getItem('rvt-density') as any;
      if (densityVal) this.density.set(densityVal);
      ...
      const hxModeVal = localStorage.getItem('rvt-hx-mode') as any;
      if (hxModeVal) this.hxMode.set(hxModeVal);
  ...
  triggerHaptic(kind: string) {
    ...
    // Skip haptics during session except critical ones
    if (sessionActive && !['safety', 'confirm-stop', 'reject'].includes(kind)) return;
