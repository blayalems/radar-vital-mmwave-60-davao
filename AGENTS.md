...
| `radar_vital_v16_2_0.ino` | Firmware. `ENABLE_BLE` is `false` by default — keep v15 behavior identical until Phase 4F. Watchdog must never `delay()` in the DSP loop; use `bleSuppressUntilMs = millis() + N` instead. |
