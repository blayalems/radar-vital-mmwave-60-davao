...
| -- | ---: | ------- | ------ | --- | ------ | -------- | --- | ----- |
| R1 | 1 | `build-exe.yml` firmware path filter referenced non-existent `radar_vital_v16_2_0.ino`, so firmware edits never triggered the EXE gate | CGPT, Claude, Genspark, Arena, Jules, Grok Fast | P1 | **fixed** | `.github/workflows/build-exe.yml:15`; current real file is `radar_vital_v16_4_0.ino` | Changed filter to glob `radar_vital_v16_*.ino` | `test_workflow_firmware_paths_resolve` (scans every workflow `*.ino` filter, asserts each matches a real file) |
...
| R7 | 4 | Preflight firmware guidance told operators to install `radar_vital_v15_0_0.ino` / "v15.0.0 firmware" — a file that does not exist and whose 207-col schema the current 222-col contract check rejects | Jules, GF3.5 Ext, G3.1 Pro Ext, data re-validation | P2 | **fixed** | `rvt_trainer/audit/runner.py:94,102` | Point to `radar_vital_v16_4_0.ino` / the v15.2 222-column contract while keeping 219-column v15.1 replay compatibility | `test_preflight_firmware_guidance_points_to_current_firmware` |
...
| D3 | 7 | Local firmware-flash path exists (`cmd_flash` → `arduino-cli upload`); the radar→trainer serial frame is plain ASCII CSV with **no per-line CRC / sequence / length** | Oppie, GF3.5 Ext, G3.1 Pro Ext, Grok Fast | P2 | partially-confirmed / blocked-by-hardware | `monolith.py` `cmd_flash`; `radar_vital_v16_4_0.ino`; `transport/serial.py:73-86` | Host-side CRC/seq detection requires a firmware-side change + a real device to validate; the `"DATA,"` marker + v15.2 222-col contract + `to_numeric(coerce)` give partial robustness today |
| D4 | 7/13 | `cmd_predict` does `pickle.load` of `model_hr/rr.pkl` + `preprocessor.pkl` from `--model-dir` with no path/realpath check, no signature/hash → arbitrary-code-on-load from an untrusted model dir | Claude, Jules, Grok Fast, Oppie, Sonnet Search | P1 (local-trust) | fixed (PR #58 + strict-mode follow-up) | `monolith.py` `_verify_model_dir` (realpath + sha256 + structural artifact-set check + HMAC); CLI arg `--model-dir` | Fixed via a signed model manifest verified before `pickle.load`. **Two integrity levels:** an *unsigned* manifest (sha256 only) is self-attesting — it detects accidental corruption / local drift but NOT a tampering attacker (who can rewrite both the `.pkl` and its recorded hash). `RVT_REQUIRE_MODEL_SIGNATURE=1` enables **strict mode** = tamper/authenticity protection: requires `RVT_MODEL_SIGNING_KEY` **and** an `hmac_sha256` signature in the manifest, verified with `hmac.compare_digest`, failing closed on missing key, absent signature, or HMAC mismatch (also implies manifest-required). `RVT_REQUIRE_MODEL_MANIFEST=1` only requires a manifest to *exist*, not to be authenticated |
