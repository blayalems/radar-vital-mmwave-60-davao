"""radar_vital_trainer_v8_5_6_for_v13_9_6.py â€” v8.5.6
=====================================================
Leakage-aware offline trainer + full session pipeline for the
XIAO ESP32-C6 + MR60BHA2 system.

v8.5.6 keeps the v8.5.5 contract and adds the v13.9.6 truthfulness-and-semantics release with RR source/freshness analysis, trust-age telemetry, stronger masking, and dashboard support.
...
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  â€¢ v13.9.6 telemetry contract: v13.9.5 fs diagnostics plus RR source/freshness, trust-age, dead-zone run-length, module-valid, and publish-stage telemetry.
  â€¢ trainer validity masks now add phase-run, AGC, freshness, and RR source-latch guards, and emit explicit audit masks for physiology-vs-policy analysis.
  ...
  â€¢ dashboard loader prefers an external v8.5.6 HTML template when present
  â€¢ contributions blended from Opus 4.6/4.7, Sonnet 4.5/4.6, ChatGPT 5.4 Codex/Pro, Gemini, Grok, GLM, Muse, DeepSeek, and Qwen audits, with only code-backed or low-risk changes applied.

Primary workflow
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  1.  python radar_vital_trainer_v8_5_6_for_v13_9_6.py doctor
  2.  python radar_vital_trainer_v8_5_6_for_v13_9_6.py quickstart

  Per session (single command):
      python radar_vital_trainer_v8_5_6_for_v13_9_6.py session \
        --port COM10 \
  ...
  After session:
      python radar_vital_trainer_v8_5_6_for_v13_9_6.py compare --sessions-dir sessions/ --out report.html
...
HR_PUBLISH_REASON_NAMES = {0:"OK",1:"NO_HUMAN",2:"TRUST_STALE",3:"NO_EVIDENCE",4:"STATE",5:"AGE",6:"STALE",7:"PQI",8:"CONF",9:"HARMONIC",10:"RAW_DISAGREE",11:"OTHER",12:"GRACE_BLOCKED",13:"PHASE_HOLDOFF"}
...
EXPECTED_RADAR_LOG_COLUMN_COUNT = 177
...
def _truthfulness_from_radar(raw_df: pd.DataFrame) -> Dict[str, object]:
    ...
    if len(mod_major) and len(mod_sub) and len(mod_mod):
        ...
        out["module_version_valid"] = out["module_version_valid"] or (module_version is not None)
    return out
...
<title>Radar Vital Trainer Live Dashboard - v13.9.6 / v8.5.6 Guarded Experimental Audit</title>
...
<body>
  <div class="page">
      ...
      <div class="row">
        <div class="brand">
          ...
          <div>
            ...
            <div class="subtitle">v13.9.6 / v8.5.6 dashboard with guarded experimental profile, audit-only exclusions, and policy-hardened live telemetry</div>
          </div>
        </div>
    ...
    <section class="grid cards-2">
      <article class="card">
        <div class="card-header">
          <div class="title-wrap"><span class="material-symbols-rounded">conversion_path</span><div><h2 class="card-title">HR funnel telemetry</h2><div class="card-subtitle">Arbiter â†’ reject-phase â†’ blend â†’ coherence â†’ publish</div></div></div>
        </div>
    ...
    <section class="grid cards-2">
      <article class="card">
        <div class="card-header">
          <div class="title-wrap rr"><span class="material-symbols-rounded">air</span><div><h2 class="card-title">RR funnel telemetry</h2><div class="card-subtitle">Selector â†’ accept-phase â†’ blend â†’ bias correction â†’ Kalman â†’ publish</div></div></div>
        </div>
  ...
  <script>
    ...
    function render(payload){
      ...
      document.getElementById('bleTable').innerHTML = [
        ...
        makeRow('SpOâ‚‚', ble.spo2 === undefined ? '--' : `${fmt(ble.spo2,0)}%`),
      ...
      const cards = [];
      if (hrDelta !== null) cards.push(faultCard(Math.abs(hrDelta) > hrCrit ? 'bad' : (Math.abs(hrDelta) > hrWarn ? 'warn' : 'good'), 'Heart-rate mismatch', `Radar ${fmt(radarHr,1)} vs BLE ${fmt(bleHr,1)} â†’ Î” ${fmt(hrDelta,1)} bpm.`));
      if (rrDelta !== null) cards.push(faultCard(Math.abs(rrDelta) > rrCrit ? 'bad' : (Math.abs(rrDelta) > rrWarn ? 'warn' : 'good'), 'Respiration mismatch', `Radar ${fmt(radarRr,1)} vs BLE ${fmt(bleRr,1)} â†’ Î” ${fmt(rrDelta,1)} br/min.`));
      ...
      document.getElementById('hrFunnelPills').innerHTML = [
        ...
        pill('Trust fresh', boolish(getRadar(payload,'trusted_hr_fresh')) ? 1 : 0),
...
for _dashboard_candidate in _dashboard_template_candidates():
    if _dashboard_candidate.exists():
        try:
            ...
            pass
_DASHBOARD_TEMPLATE_EMBEDDED = _DASHBOARD_TEMPLATE_EMBEDDED.replace("v13.8.0 / v8.4.0", "v13.9.6 / v8.5.6")
...
def _firmware_contract_candidates() -> List[Path]:
    ...
    relatives = [
        Path("radar_vital_v13_9_6.ino"),
        Path("radar_vital_v13_9_6") / "radar_vital_v13_9_6.ino",
...
STEP 0 â€” Check your setup
  python radar_vital_trainer_v8_5_6_for_v13_9_6.py doctor
...
STEP 3 â€” Compare sessions after you collect several runs
  python radar_vital_trainer_v8_5_6_for_v13_9_6.py compare \
    --sessions-dir sessions/ --out report.html

STEP 4 â€” Train a correction model after baseline quality is acceptable
  python radar_vital_trainer_v8_5_6_for_v13_9_6.py train \
...
Manual radar logging:
  python radar_vital_trainer_v8_5_6_for_v13_9_6.py log --port COM10 --out sessions/s01/radar.csv

Manual BLE reference logging:
  python radar_vital_trainer_v8_5_6_for_v13_9_6.py ble_reflog \
...
Manual analysis:
  python radar_vital_trainer_v8_5_6_for_v13_9_6.py analyse \
    --radar sessions/s01/radar.csv --ref sessions/s01/ref.csv \
    --out sessions/s01/analysis/

DSP sweep:
  python radar_vital_trainer_v8_5_6_for_v13_9_6.py sweep \
    --sweep-json params_sweep.json \
    --port COM10 --sketch path/to/radar_vital_v13_9_6.ino \
    --out-dir sweep_results/

Run  python radar_vital_trainer_v8_5_6_for_v13_9_6.py doctor  to verify your environment.
...
def cmd_doctor(args):
    ...
    if ok_all:
        ...
        print(f"Run  {_bold('python radar_vital_trainer_v8_5_6_for_v13_9_6.py quickstart')}  for the workflow guide.\n")
    else:
...
def pick_feature_columns(
    ...
    cols = [
        c for c in cols
        if c not in POLICY_FEATURE_EXACT and
           not any(c == pref or c.startswith(pref + "_") for pref in POLICY_FEATURE_PREFIXES)
    ]
...
def validity_masks(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rr_source_latched_ok_mask = (_pick("rr_source_latched_ok_last", "rr_source_latched_ok_mean", 1.0) >= 0.5)
    ...
    base_rr = base_rr & trusted_rr_mask & warm_mask & agc_mask & rr_source_latched_ok_mask & post_motion_mask
...
def cmd_analyse(args):
    ...
    if _safe_int(primary_gate.get("n"), 0) < 15:
    ...
    if primary_gate.get("status") == "INSUFFICIENT_DATA":
...
def cmd_align(args):
    ...
    print(f"\nNext step:")
    print(f"  python radar_vital_trainer_v8_5_6_for_v13_9_6.py analyse --radar {args.radar[0]} "
...
def cmd_sweep(args):
    ...
    for i, exp in enumerate(experiments, start=1):
        ...
        if args.ref:
            print(f"  {_yellow('REMINDER:')} run  python radar_vital_trainer_v8_5_6_for_v13_9_6.py reflog  in a second terminal!")
        try:
    ...
    if args.ref:
        ...
        print(f"  python radar_vital_trainer_v8_5_6_for_v13_9_6.py compare --sessions-dir {args.out_dir} --out report.html")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Quick start:\n"
            "  python radar_vital_trainer_v8_5_6_for_v13_9_6.py doctor      # check dependencies\n"
            "  python radar_vital_trainer_v8_5_6_for_v13_9_6.py quickstart  # full workflow guide\n"
        ),
    )
