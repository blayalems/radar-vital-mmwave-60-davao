...
| High | CI did not exercise stated viewport/visual/static contracts and initially targeted Win32 visual baselines from an Ubuntu runner. | Fixed: Ubuntu CI runs complete Python and Playwright functional/API viewport checks; reviewed `-win32.png` visual baselines run on `windows-latest`, matching their capture platform. Native build workflows retain artifact gates. |
...
- `python -m pytest -q tests` passed with 36 tests, including LAN authorization/static denial, Pages Angular-shell publication, CI snapshot-platform alignment, and frozen 207-column/115200 protocol assertions.
...
- `npx playwright test tests/visual/rvt-v12.spec.ts` passed all 80 current route/theme/device baselines after intentional HC/settings updates; all 84 committed snapshot files are Win32-targeted and CI visual comparison now runs on `windows-latest`.
