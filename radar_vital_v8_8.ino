...
void respirationLockedClean(float *buf, int n, float fs, float rrBPM) {
  ...
  for(int h = 2; h <= 4; h++) {
    ...
    float amp = 2.0f * sqrtf(re*re + im*im) / n;
...
float autocorr(float *buf, int n, int lag) {
  float sum = 0;
  for(int i = 0; i < n - lag; i++) sum += buf[i] * buf[i + lag];
  return sum;
}
...
void loop() {
  ...
  if(smoothRR >= RR_MIN) {
    if(fabsf(smoothRR - lastNotchRR) > 0.5f) {
  ...
  float breathGate      = fabsf(lut_cos(breathInstPhase));
  ...
  if(lcdConnected && millis() - lastDisplay > DISPLAY_INTERVAL) {
    ...
    if(humanDetected) {
      char line1[18];
      if(isnan(smoothTemp))
        snprintf(line1, 17, "HR%3d RR%2d T--.-",
                 (int)smoothHR, (int)smoothRR);
      else
        snprintf(line1, 17, "HR%3d RR%2d T%4.1f",
