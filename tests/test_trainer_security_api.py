...
def test_api_version_additive_product_and_schema_fields(tmp_path: Path):
    ...
    try:
        ...
        assert status == 200
        assert payload["trainer"] == "16.6.1"
        assert payload["dashboard"] == "16.6.1"
        assert payload["product_version"] == "16.6.1"
        assert payload["firmware_expected"] == "v16.6.1"
...
def test_update_manifest_proxy_success(tmp_path: Path):
    ...
    try:
        with patch("urllib.request.urlopen", side_effect=side_effect):
            ...
            assert status == 200
            assert data["product_version"] == "16.6.1"
    finally:
        server.stop()
