...
test.describe('OTA Update and Version Info Smoke Tests', () => {
  ...
  test('displays product version and metadata in settings', async ({ page }) => {
    ...
    await expect(productVersion).toContainText('16.6.0');
  ...
  test('checks for updates successfully when a newer version is available', async ({ page }) => {
    ...
    await page.route('**/api/update/manifest', async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          artifacts: {
            apk: {
              url: 'https://github.com/blayalems/radar-vital-mmwave-60-davao/releases/download/v16.6.1/radar-vital-release.apk',
              size_bytes: 15728640,
              sha256: 'a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2',
              compatibility: 'Android 8.0+'
            },
            exe: {
              url: 'https://github.com/blayalems/radar-vital-mmwave-60-davao/releases/download/v16.6.1/radar-vital-windows-installer.exe',
              size_bytes: 26214400,
    ...
    await expect(titleText).toContainText('Update Available: v16.6.1');
    ...
    await expect(page.locator('simple-snack-bar').last()).toContainText('New update 16.6.1 is available.');
  });
  ...
  test('checks for updates successfully when a newer build of the same version is available', async ({ page }) => {
    ...
    await page.route('**/api/update/manifest', async (route) => {
      await route.fulfill({
        ...
        contentType: 'application/json',
        body: JSON.stringify({
          product_version: '16.6.0',
          release_tag: 'v16.6.0-build2',
          ...
          artifacts: {
            apk: {
              url: 'https://github.com/blayalems/radar-vital-mmwave-60-davao/releases/download/v16.6.0-build2/radar-vital-release.apk',
              size_bytes: 15728640,
              sha256: 'a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2',
              compatibility: 'Android 8.0+'
            }
    ...
    const titleText = resultRegion.locator('.update-title-text');
    await expect(titleText).toContainText('New Build Available: v16.6.0-build2');

    // Verify snackbar alert
    await expect(page.locator('simple-snack-bar').last()).toContainText('New build of 16.6.0 is available.');
  });
