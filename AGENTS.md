...
| `radar_vital_v16_5_3.ino` | Firmware. `ENABLE_BLE` is `false` by default; the active USB contract is v15.2 / 222 columns. Keep radar BLE/GATT work gated until accepted. Watchdog must never `delay()` in the DSP loop; use `bleSuppressUntilMs = millis() + N` instead. |
...
10. **Feedback-loop export clean.** `npm run docs:export-feedback-loop -- --check` confirms both canonical Mermaid figures and manuscript caption snippets can be reproduced from `docs/system-feedback-loop.md`.
11. **PR release step valid.** On pull requests, `npm run test:pr-version -- --base-ref origin/$GITHUB_BASE_REF` proves the branch advances exactly one patch or minor version from its base.
